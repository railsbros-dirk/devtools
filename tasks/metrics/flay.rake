...
namespace :metrics do
  begin
    ...
    if ! compatible_scores.include?(Triage.rvm)
      task :flay do
        $stderr.puts "Flay is disabled under #{Triage.rvm}"
      end
      ...
      task :flay do
        ...
        unless max >= threshold
          Triage.notify "Adjust flay threshold down to #{max}"
        end

        total = masses.inject(:+).to_i
        unless total == total_score
          Triage.notify "Flay total is now #{total}, but expected #{total_score}"
        end
        ...
        if mass_size.nonzero?
          flay.report
          Triage.notify "#{mass_size} chunks have a duplicate mass > #{threshold}"
        end
      end
    else
      task :flay do
